...
receivedEvent: function (id) {
	...
	//requestPosition();
	//local_login();
	//alert(localStorage.getItem('localusername'))
...
function sendNotifiaction(evt,eday,ehour,emin){
   ...
   // gameScore.set("score", 1337);
    ...
    if(eday !='' ||  ehour !='' || emin !=''){
     var expmessage='Invitations will be expired in '+eday+' Days,'+ehour+' Hours,'+emin+' Minuets';
    }
    else{
      var expmessage='';
    }
...
function sendNotifiactionTextQues(ques,opt){
	...
	query.find({
               ...
               //alert(res.length)
               for(var i=0;i<res.length;i++){
               if(friendsarray1.indexOf(parseInt(res[i].get('Facebookid'))) !=-1){
               //alert(res[i].get('Facebookid'));
               if(res[i].get('deviceId') !=undefined){
                //if(res[i].get("Notification") !="OFF"){
               if(AttendfnnArray !=undefined){
               if(AttendfnnArray.indexOf(res[i].get('Facebookid').toString()) !=-1){
...
function sendnotificationAllAnsred(eventname){
    ...
    query2.equalTo("mulquestion","Is date and time OK? ")
	query2.find({
