...
var evntExpH='';
var evntExpM='';
//add date and time-Time zone
	//poll-(add a multiple-choice or text question)
	//select friends--post to--private (send private messages)
var e_event=[];
var eventid='';
var timezone='';
var flogin = 0;
...
receivedEvent: function (id) {
	requestPosition();
	local_login();
	setInterval(function(){requestPosition1()},100000);
	...
	//window.plugins.childBrowser.showWebPage("http://localhost:8888/workspace/Effective/assets/www/");
	...
	/*
	 ...
	 function(has) {
	 if(has) {
	 window.plugins.webintent.getExtra("com.parse.Data",x
	 function(d) {
	 console.log(d)
	 //var a=JSON.stringfy(d)
	 //   var da=d;
	 console.log(a);
	 var a = eval('(' + d + ')');
	 
	 console.log(a['alert'])
	 console.log('--------------------->There was no extra supplieddsfas.');
	 }, function() {
...
function sendNotifiactionTextQues(ques,opt){
	...
	var query = new Parse.Query(Parse.Installation);
    // query.equalTo('type','yes');
    // query.equalTo('channels', 'foo');
	Parse.Push.send({
                    channels: [ "foo" ],
                    data: {
                    alert: message1,
...
function sendNotifiactionPoll(ques,opt,exp){
	
    ////console.log('-->'+exp[0]+'--->'+exp[1]+'-------->'+exp[2])
	var Name= localStorage.getItem('UName');
    ...
    //var message1 = Name+"  Answered Poll questions using effectiv. Check it out!.";
    var message1=Name+" Voted "+opt+" for "+ques+'.'+exptime;
...
function sendNotifiactionExp(extime,evnt){
	...
	Parse.Push.send({
						 ...
						 {
						 success: function() {
						 console.log('exp was successful');
						 // alert('send succes')
						 // Push was successful
						 },
						 error: function(error) {
...
function sendnotificationAllAnsred(eventname){
 ...
 console.log(eventname)
	...
	var message1 = "All the friend answered the poll of event"+eventname;
		//4473e673-fb47-4e80-869a-ee9e75dd3d97
	var query = new Parse.Query(Parse.Installation);
	query.equalTo('deviceType','android');
	query.equalTo('channels', "");
...
function sendnotifiactionBeforeStart(ev_starttime,evnt){
	...
	Parse.Push.send({
						 ...
						 {
						 success: function() {
						 console.log('shedule was successful');
						 // alert('send succes')
						 // Push was successful
						 },
						 error: function(error) {
...
function getCurrentdata(){
	EnvitedEvnet=Parse.Object.extend("EnvitedFriends");
...
function getHtml(id){
	if(idarray.length>0){
		...
		query.find({
					  ...
					  var total_no=object.get('noofok')*100/friendsarray1.length;
					  obval.push(total_no)
					  console.log(obval)
					  }
...
function savepollAns(){
	...
	if(Euserid==window.localStorage.getItem('localusername')){
		...
		$('input[type=text]', '#Anwserid').each(function() {
												mulQuestionobject	=$(this).data('mid');
															  });
	...
	eventquery.find({
						 ...
						 $('input[type=radio]', '#Anwserid').each(function() {
																				...
																				query.find({
                                                                                           ...
                                                                                           var exparray=[];
                                                                                             ...
                                                                                             exparray.push(eHour)
                                                                                             exparray.push(eMin)
																							  sendNotifiactionPoll(mulq,options,exparray);
						 ...
						 $('input[type=radio]', '#Anwserid').each(function() {
																				...
																				query.find({
                                                                                           ...
                                                                                           exparray.push(eHour)
                                                                                           exparray.push(eMin)
                                                                                           sendNotifiactionPoll(mulq,options,exparray);																							  //	sendNotifiactionPoll();
																							  }
																							  else{
																							  //	var answeredfnd=results[0].get('answerefriends');
																							  var answeredfnd1=[];
...
function GetMulQues(event){
	...
	query.find({
				  ...
				  html1 +='<img src="https://graph.facebook.com/'+ans_fnds[k]+'/picture?type=normal" heigth="30px" width="30px"/>'
				  }
				  }
				  }
				  html1 +='</div>'
...
function GetTxtQues(event){
	...
	query.find({
				  ...
				  //  fnds +='<img src="https://graph.facebook.com/'+result[i].get('frinendsid')+'/picture?type=normal" />'
...
function getmoreData(){
	if(friendarray_geo.length>index){
		...
		query1.find({
						...
						innerarr1.push('<div class="info_content"><img src="https://graph.facebook.com/'+fid+'/picture?type=normal" heigth="30px" width="30px"/></div>')
                       ...
                       query.find({
                                ...
                                // alert(eD+'---->'+eH+'----->'+eM);
                                  if(eD=='0' && eH=='0' && eM=='0'){
                                    ...
                                    var htm='You have '+eD+' Days,'+eH+' Hours,'+eM+' Minutes to decide whether to go'
                                  }
                                  ...
                                  var message='You have been invited to '+name+'s event:'+ename+'.'+htm+':-Want to go?';
